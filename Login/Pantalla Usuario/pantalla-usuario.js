...
function openAddModal() {
  selectType("Carro");
  ["addPlate", "addBrand", "addColor", "addYear"].forEach(
...
function selectType(type) {
  ...
  document.getElementById("fieldPlacaLabel").textContent = isBici
    ? "Número de Serie / Marco"
    : "Placa";
  document.getElementById("addPlate").placeholder = isBici
    ? "Ej: SER-2023-001"
    : "Ej: ABC-123";
  document.getElementById("fieldYearGroup").style.display = isBici
    ? "none"
    : "block";
}
...
function saveNewVehicle() {
  ...
  if (!isBici && !year) {
    showToast("Ingresa el año");
    return;
  }
  if (
    state.vehicles.find((v) => v.plate.toUpperCase() === plate.toUpperCase())
  ) {
    showToast("Ese vehículo ya está registrado");
    return;
  }
  state.vehicles.push({
  ...
  showToast(
    isBici
      ? "Bicicleta registrada · Valida con el operador al ingresar"
      : "Vehículo registrado · La cámara IA lo reconocerá",
  );
}
...
function makeReservation() {
  ...
  if (!vehicle) {
    showToast("Registra un vehículo primero");
    return;
  }
  if (state.activeReservation) {
    showToast("Ya tienes una reserva activa");
    return;
  }
  if (payment === "prepaid") {
    if (
...
function toggleProfileForm(show) {
  document.getElementById("profileDisplay").style.display = show
    ? "none"
    : "block";
  document.getElementById("profileForm").style.display = show
    ? "block"
    : "none";
}
...
function changePassword() {
  ...
  ["passOld", "passNew", "passConfirm"].forEach(
    (id) => (document.getElementById(id).value = ""),
  );
